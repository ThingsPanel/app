<template>
  <view class="time-trigger" @tap="open">
    <text class="time-trigger-value" :class="{ placeholder: !modelValue }">{{ displayValue || placeholder || $t('pages.sceneAutomationEditor.selectTime') }}</text>
    ...
    <view class="time-sheet" @tap.stop>
      ...
      <picker-view class="time-picker" :value="pickerValue" indicator-style="height:48px" @change="onPickerChange">
        <picker-view-column v-if="!minuteOnly">
          <view v-for="hour in hours" :key="hour" class="time-option">{{ pad(hour) }} {{ $t('pages.sceneAutomationEditor.hourUnit') }}</view>
        </picker-view-column>
        <picker-view-column>
          <view v-for="minute in minutes" :key="minute" class="time-option">{{ pad(minute) }} {{ $t('pages.sceneAutomationEditor.minuteUnit') }}</view>
        </picker-view-column>
      </picker-view>
    </view>
  </view>
...
export default {
  ...
  computed: {
    ...
    displayValue() {
      ...
      return this.minuteOnly ? this.$t('pages.sceneAutomationEditor.minuteValue', { minute: date.getMinutes() }) : `${this.pad(date.getHours())}:${this.pad(date.getMinutes())}`;
    }
  },
  methods: {
...
<style scoped>
.time-trigger { display:flex; align-items:center; justify-content:space-between; box-sizing:border-box; width:100%; min-height:48px; gap:8px; color:#1d1d1f; font-size:13px; }
