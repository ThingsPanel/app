...
const bufferedSchema = { ...schema, dataSources: [{ ...schema.dataSources[0], config: { ...schema.dataSources[0].config, bufferSize: 100 } }] }
assert.equal(collectDeviceHistory(bufferedSchema, { dataSourceId: 'device', fieldIds: ['power'] }).size, 0, '普通字段不应因为开启数据源缓冲而查询历史')
assert.equal(collectDeviceHistory(bufferedSchema, { dataSourceId: 'device', fieldIds: ['power__history'] }).get('power'), 'last_30d', '明确请求历史字段时仍应查询历史')
