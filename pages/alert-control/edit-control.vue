	...
	<view class="tp-box tp-box-sizing tp-flex tp-flex-col">
		...
		<view class="tp-panel tp-flex tp-flex-col tp-mg-l-r-30">
			...
			<view class="switchBtn">
				<view class="on">
					<view class="onIcon" v-if="switchOnOne == 0" @click="switchOnOne=1;switchCloseOne=0"></view>
					<view class="closeIcon" v-else @click="switchOnOne=0;switchOnOne=1">
						<view class="closeIconL"></view>
					</view>
					<i style="font-style: normal;">开</i>
				</view>
				<view class="close">
					<view class="closeIcon" v-if="switchCloseOne == 1&&switchOnOne == 0"
						@click="switchCloseOne=0;switchOnOne=1">
						<view class="closeIconL"></view>
					</view>
					<view class="onIcon" v-else @click="switchCloseOne=1;switchOnOne=0"></view>关
				</view>
			</view>
		</view>
		...
		<view class="tp-panel tp-flex tp-flex-col tp-mg-l-r-30">
			<view class="tp-flex tp-flex-row tp-flex-j-s tp-flex-a-c tp-box-sizing tp-pd-t-b-25">
				...
				<view class="tp-flex-1 tp-flex tp-flex-row tp-flex-j-r tp-flex-a-c">
					<view class="switchBtn">
						<view class="on">
							<view class="onIcon" v-if="switchOnTwo == 0" @click="switchOnTwo=1;switchCloseTwo=0"></view>
							<view class="closeIcon" v-else @click="switchOnTwo=0;switchOnTwo=1">
								<view class="closeIconL"></view>
							</view>
							<i style="font-style: normal;">开</i>
						</view>
						<view class="close">
							<view class="closeIcon" v-if="switchCloseTwo == 1&&switchOnTwo == 0"
								@click="switchCloseTwo=0;switchOnTwo=1">
								<view class="closeIconL"></view>
							</view>
							<view class="onIcon" v-else @click="switchCloseTwo=1;switchOnTwo=0"></view>关
						</view>
					</view>
				</view>
			</view>
...
<script>
	export default {
		...
		methods: {
			getDetail() {
				this.API.apiRequest('/automation/update', {
					id: this.editId
				}, 'post').then(res => {
					if (res.code == 200) {
						const data = res.data
						...
						this.sort = data.sort
						this.currentClType.id = data.type
						this.clTypeList.forEach(item => {
							if (item.id == data.type) {
								this.currentClType = item
							}
						})
						this.currentZcOne.id = data.config.rules[0].asset_id
						this.currentEqpSx.key = data.config.rules[0].field
						this.currentEqpOne.id = data.config.rules[0].device_id
						this.currentGx.name = data.config.rules[0].condition
						this.warningNum = data.config.rules[0].value
						this.currentZcTwo.id = data.config.apply[0].asset_id
						this.currentSkdx.key = data.config.apply[0].field
						this.currentEqpTwo.id = data.config.apply[0].device_id
						var switchOnOne, switchOnTwo;
						if (data.config.apply[0].value == 0) {
							switchOnOne = 0
						} else {
							switchOnOne = 1
						}
						...
						this.switchOnOne = switchOnOne
						this.switchOnTwo = switchOnTwo
						this.$forceUpdate()
						this.getZcList()
			...
			doUpdateSubmit() {
				if (this.validate()) {
					...
					this.API.apiRequest('/automation/edit', params, 'post').then(res => {
						if (res.code == 200) {
							this.getDetail()
							uni.showToast({
								title: res.message
			...
			getEqpType() {
				...
				this.API.apiRequest('/automation/show', {
					bid: this.currentEqpOne.id
				}, 'post').then(res => {
					if (res.code == 200) {
						this.eqpSxList = res.data
						res.data.forEach(item => {
							if (item.key == this.currentEqpSx.key) {
			...
			getEqpTypeTwo(id) {
				...
				this.API.apiRequest('/automation/instruct', {
					bid: this.currentEqpTwo.id
				}, 'post').then(res => {
					if (res.code == 200) {
						this.eqpSxListTwo = res.data
						res.data.forEach(item => {
							if (item.key == this.currentSkdx.key) {
								this.currentSkdx = item
							}
			...
			getEpqDataOne() {
				...
				this.API.apiRequest('/device/index', {
					...
					if (res.code == 200) {
						...
						res.data.forEach(item => {
							if (this.currentEqpOne.id == item.id) {
								this.currentEqpOne = item
							}
						})
						if (this.currentZcTwo.id) {
							this.getEpqData()
						}
						// this.getZcList()
					}
				...
				if (this.currentZcTwo.id == undefined) {
					...
					this.API.apiRequest('/device/index', {
						...
						if (res.code == 200) {
							...
							res.data.forEach(item => {
								if (this.currentEqpTwo.id == item.id) {
									this.currentEqpTwo = item
								}
							})
							if (this.currentEqpOne.id) {
								this.getEqpType()
							}
							if (this.currentEqpTwo.id) {
								this.getEqpTypeTwo()
							}
				...
				this.API.apiRequest('/automation/property', {
					...
					if (res.code == 200) {
						...
						this.zcList.forEach(item => {
							if (item.id == this.currentZcOne.id) {
								this.currentZcOne = item
							}
							if (item.id == this.currentZcTwo.id) {
								this.currentZcTwo = item
							}

						})
						if (this.currentZcOne.id) {
							this.getEpqDataOne()
						}
					}
					uni.hideLoading()
