export default {
  scanActivation: { scan: '扫码添加设备', appOnly: '请在 App 中使用扫码添加设备', empty: '未识别到设备编号，请重新扫码', missing: '缺少设备编号，请返回设备列表重新扫码', failed: '设备激活失败，请重试' },
  dashboard: {
    "title": "首页",
    "tenantOverview": "租户总览",
    "updating": "正在更新",
    "overviewSubtitle": "设备与告警概览",
    "notifications": "告警通知",
    "deviceTotal": "设备总数",
    "onlineCount": "在线 {count} 台",
    "onlineRate": "设备在线率",
    "alarmDevices": "告警设备",
    "viewActivity": "查看动态",
    "loadFailed": "{sections}加载失败，点击重试",
    "operations": "运营概览",
    "updatedAt": "更新于 {time}",
    "refresh": "刷新",
    "todayAlarms": "今日告警",
    "automationRules": "联动规则",
    "shortcuts": "快捷入口",
    "alarmActivity": "告警动态",
    "viewAll": "查看全部",
    "alarmsUnavailable": "告警动态暂不可用",
    "noAlarms": "暂无告警记录",
    "alarmRecord": "告警记录",
    "viewDetails": "查看详情",
    "commonDevices": "常用设备",
    "commonDevicesUnavailable": "常用设备暂不可用",
    "noDevices": "暂无设备",
    "typeDirect": "直连",
    "typeGateway": "网关",
    "typeSubDevice": "子设备",
    "groupStatus": "分组状态",
    "groupsUnavailable": "分组状态暂不可用",
    "noGroups": "暂无设备分组",
    "statisticsUnavailable": "统计暂不可用",
    "groupAlarmCount": "告警设备 {count}",
    "noAlarmDevices": "无告警设备",
    "groupCounts": "设备 {total} · 在线 {online}",
    "deviceStatistics": "设备统计",
    "groupStatistics": "分组统计",
    "devices": "设备",
    "alarms": "告警",
    "automation": "自动化",
    "boards": "看板",
    "groups": "分组",
    "rules": "告警规则",
    "scenes": "场景",
    "account": "我的",
    "openFailed": "页面打开失败，请重试",
    "scanInApp": "请在 App 中扫码，或使用添加设备",
    "scanFailed": "扫码失败，请重试",
    "high": "高",
    "medium": "中",
    "low": "低",
    "recovered": "已恢复",
    "unknown": "未知",
    "alarmOpenFailed": "告警详情打开失败"
},

  common: {
    confirm: '确认',
    cancel: '取消',
    save: '保存',
    delete: '删除',
    edit: '编辑',
    loading: '加载中...',
    pleaseSelect: '请选择',
    pleaseInput: '请输入',
    none: '无',
    close: '关闭',
    ok: '确定',
    search: '搜索',
    status: '状态',
    noData: '暂无数据',
  },
  login: {
    title: '登录',
    username: '用户名',
    password: '密码',
    loginBtn: '登录',
    registerBtn: '注册'
  },
  account: {
    securitySettings: '安全设置',
    changePassword: '修改密码',
    oldPassword: '原密码',
    newPassword: '新密码',
    confirmPassword: '确认新密码',
    passwordHint: '至少 6 位，包含小写字母和数字，具体规则由服务器校验。',
    passwordRequired: '请填写全部密码字段',
    passwordMismatch: '两次输入的新密码不一致',
    passwordSuccess: '密码修改成功',
    passwordFailed: '修改失败，请检查原密码和新密码要求后重试',
    basicInfo: "基础信息",
    accountType: "账户类型",
    tenantAdmin: "租户管理员",
    tenantUser: "租户用户",
    systemAdmin: "系统管理员",
    notSet: "未设置",
    region: "省市区",
    detailAddress: "详细地址",
    preferences: "偏好设置",
    appLanguage: "应用语言",
    retryLoad: "资料加载失败，点击重试",
    notLoggedIn: '未登录/注册',
    clickToLogin: '点击头像可登录/注册',
    language: '语言',
    name: '名字',
    phone: '电话',
    email: '邮箱',
    logout: '退出登录',
    logoutConfirm: '退出后需要重新输入账号和密码才能继续使用。',
    logoutFailed: '退出登录失败，请重试',
    deleteAccount: '永久注销账号',
    serverAddress: '服务器地址',
    serviceConfig: '服务配置',
    addressConfigured: '已配置',
    addressDefault: '默认',
    enterServerAddress: '请输入服务器地址',
    warning: '警告',
    deleteConfirmation: '账号注销后数据无法恢复，您在系统中注册的个人信息也将永久删除！请确认注销账号！',
    deleteSuccess: '注销成功',
    deleteFailed: '注销失败',
    loading: '加载中',
    tip: '提示',
    enterServerAddressToast: '请输入服务器地址',
    confirm: '确定',
    authDescription: '登录授权',
    edit: {
      title: '编辑个人信息',
      avatar: '头像',
      address: '地址信息',
      province: '省份',
      city: '城市',
      district: '区县',
      provincePlaceholder: '请输入省份',
      cityPlaceholder: '请输入城市',
      districtPlaceholder: '请输入区县',
      detailedAddressPlaceholder: '请输入详细地址',
      changeAvatar: '更换头像',
      namePlaceholder: '请输入姓名',
      nameRequired: '姓名不能为空',
      phonePlaceholder: '请输入手机号码',
      phonePrefixPlaceholder: '前缀',
      phoneInvalid: '请输入正确的手机号码',
      emailPlaceholder: '请输入邮箱地址',
      emailInvalid: '请输入正确的邮箱地址',
      organization: '组织',
      organizationPlaceholder: '请输入组织名称',
      timezone: '时区',
      timezonePlaceholder: '请输入时区',
      defaultLanguage: '默认语言',
      defaultLanguagePlaceholder: '请输入默认语言',
      uploading: '上传中...',
      uploadSuccess: '上传成功',
      uploadFailed: '上传失败',
      updateSuccess: '更新成功',
      saveSuccess: '保存成功',
      saveFailed: '保存失败',
      updateFailed: '更新失败',
      loadFailed: '加载失败'
    }
  },
  pages: {
    loginTitle: '登录',
    register: '注册',
    alarmsTitle: '告警信息',
    alarmDetailTitle: '告警详情',
    deviceList: '设备',
    automationTitle: '自动化',
    dashboardTitle: '首页',
    accountTitle: '个人中心',
    deviceDetailTitle: '设备详情',
    addDevice: '添加设备',
    historyOperation: '历史操作',
    myDevices: '我的设备',
    alarmRules: {
      eyebrow: 'ALARM POLICY',
      title: '告警规则',
      create: '新增规则',
      searchPlaceholder: '按规则名称搜索',
      emptyTitle: '还没有告警规则',
      emptyDescription: '先定义告警等级和通知方式，再在自动化中引用。',
      createFirst: '创建第一条规则',
      notificationGroup: '通知组',
      noNotificationGroup: '不发送通知',
      loadMore: '加载更多',
      loadFailed: '告警规则加载失败',
      updateFailed: '规则状态更新失败',
      deleteTitle: '删除告警规则',
      deleteConfirm: '确定删除“{name}”吗？自动化中的引用可能受到影响。',
      deleted: '规则已删除',
      deleteFailed: '规则删除失败',
      editorKicker: 'RULE DEFINITION',
      createTitle: '新增告警规则',
      editTitle: '编辑告警规则',
      editorNavTitle: '告警规则',
      editorDescription: '规则定义告警等级和通知去向，触发条件在自动化中设置。',
      name: '规则名称',
      namePlaceholder: '例如：水温过高',
      description: '规则说明',
      descriptionPlaceholder: '说明告警的业务含义和处理建议',
      level: '告警等级',
      allLevels: '全部',
      allStatuses: '全部',
      active: '已启用',
      inactive: '已停用',
      enabled: '立即启用',
      enabledHelp: '停用后，引用此规则的自动化不会产生新告警。',
      noGroupsHelp: '当前没有可用通知组，规则仍可保存，但不会发送通知。',
      save: '保存规则',
      saved: '规则已保存',
      saveFailed: '规则保存失败',
      nameRequired: '请输入规则名称',
      levelRequired: '请选择告警等级',
      levels: {
        H: '高',
        M: '中',
        L: '低'
      }
    },
    addSceneLinkage: '新增场景联动',
    addScene: '新增场景',
    myAccount: '我的',
    deviceCreate: {
      nameHint: '为设备设置一个容易识别的名称，方便后续查找和管理。',
      pageTitle: '添加设备',
      deviceName: '设备名称：',
      deviceNamePlaceholder: '智能控制器',
      completeButton: '完成',
      deviceNameRequired: '请输入设备名称'
    },
    deviceDetail: {
      online: '在线',
      offline: '离线',
      updateTime: '更新时间',
      control: '控制',
      on: '开启',
      off: '关闭',
      send: '下发',
      logs: '日志',
      logDetail: '日志详情',
      motionDetectNoOne: '无人',
      motionDetectSomeone: '有人',
      statusClosed: '关闭',
      statusOpen: '开启',
      pullUpForMoreLogs: '上拉显示更多日志',
      noMoreLogs: '没有日志了',
      fan: '风机',
      waterCurtain: '水帘',
      lighting: '照明',
      deviceName: '设备名称',
      deviceGroupName: '设备分组名称',
      businessName: '业务名称',
      operationType: '操作类型',
      command: '指令',
      sendResult: '发送结果',
      timedTrigger: '定时触发',
      manualControl: '手动控制',
      logTitle: '日志详情',
      deviceNameLabel: '设备名称：',
      deviceGroupNameLabel: '设备分组名称：',
      businessNameLabel: '业务名称：',
      operationTypeLabel: '操作类型：',
      commandLabel: '指令：',
      sendResultLabel: '发送结果：',
      deviceMonitor: '设备监控',
      reportTime: '上报时间',
      groupSelection: '分组选择',
      timedTrigger: '定时触发',
      manualControl: '手动控制',
      success: '成功',
      failure: '失败',
      targetValue: '目标值',
      socketLinkFailure: '链接websocket失败，端口必须是9999',
      requestSendOneTimeMsg: '请求一次发送一次message',
      IllegalInput: '非法输入',
      pump1: '水泵1',
      pump2: '水泵2',
      pump3: '水泵3',
      pump4: '水泵4'
    },
    devices: {
      pageHeading: "设备",
      totalSummary: "{count} 台",
      totalUnit: "台",
      onlineRate: "在线率",
      alarmDevices: "告警设备",
      statsLoading: "正在加载统计…",
      statsRetry: "统计加载失败，点击重试",
      filter: "筛选",
      allGroups: "全部分组",
      groupPickerHint: "按分组筛选设备，支持搜索子分组",
      groupSearchPlaceholder: "搜索分组名称",
      groupCount: "共 {count} 个分组",
      childGroupCount: "{count} 个子分组",
      expandGroups: "展开全部",
      collapseGroups: "收起全部",
      allGroupsHint: "不限分组，查看全部设备",
      groupsLoading: "正在加载分组…",
      retryGroups: "重新加载",
      noGroupMatch: "未找到匹配的分组",
      tryAnotherGroupSearch: "换个关键词试试，或清除搜索",
      noGroups: "暂无可选分组",
      noGroupsHint: "当前账号未返回分组，可选择全部分组查看设备",
      currentGroup: "当前选择",
      groupsLoadFailed: "分组加载失败，请重试",

      selectGroup: "选择设备分组",
      clearGroup: "清除分组，查看全部设备",
      clear: "清除",
      gridView: "卡片视图",
      listView: "列表视图",
      backToTop: "回到顶部",
      all: "全部",
      alarm: "告警",
      title: '设备监控',
	  searchPlaceholder: '搜索设备名称或编号',
	  allDevices: '全部设备',
      online: '在线',
      offline: '离线',
	  alarming: '告警中',
	  deviceTemplate: '设备模板',
	  accessProtocol: '接入协议',
	  directDevice: '直连设备',
	  gateway: '网关',
	  subDevice: '网关子设备',
      emptyTitle: '暂无设备',
      emptyDescription: '当前没有可显示的设备',
      updateTime: '更新时间：',
      loading: '加载中',
      on: '开',
      off: '关',
      switchOn: '开启',
      switchOff: '关闭',
      stateChangeSuccess: '状态修改成功',
      pullUpForMore: '上拉显示更多数据',
      noMoreData: '没有更多数据了',
      maxFourNode: '最多可以选择4个节点'
    },
    logDetail: {
      title: '日志详情',
      deviceName: '设备名称：',
      deviceGroupName: '设备分组名称：',
      businessName: '业务名称：',
      operationType: '操作类型：',
      command: '指令：',
      sendResult: '发送结果：',
      timedTrigger: '定时触发',
      manualControl: '手动控制',
      success: '成功',
      failure: '失败'
    },
    history: {
      year: '年',
      month: '月',
      pumpOnWhenPHHigh: 'PH值>7时，自动打开水泵'
    },
    charts: {
      histogramNumber: '柱状图histogram Number',
      histogramPercent: '柱状图histogram percent',
      histogramExtra: '柱状图histogram extra拓展',
      arcbar: '圆环arcbar',
      linePercent: '折线LineChart percent',
      lineNumber: '折线LineChart Number',
      pieChart: '饼状图PieChart',
      ringChart: '环状图 RingChart',
      volume1: '成交量1',
      volume2: '成交量2',
      category1: '类别一',
      category2: '类别二',
      category3: '类别三',
      accuracy: '正确率',
      errorRate: '错误率',
      mistakeRate: '失误率',
      volumeA: '成交量A',
      volumeB: '成交量B',
      volumeC: '成交量C',
      class1: '一班',
      class2: '二班',
      class3: '三班',
      class4: '四班',
      class5: '五班'
    },
    automation: {
      sceneLinkage: '场景联动',
      sceneManagement: '场景管理',
      subtitle: '让设备按规则自动运行',
      addSceneLinkage: '新增场景联动',
      addScene: '新增场景',
      ruleDescription: '规则说明：',
      sceneDescription: '场景描述：',
      activate: '激活',
      start: '启动',
      stop: '停用',
      running: '运行中',
      disabled: '已停用',
      edit: '编辑',
      delete: '删除',
      more: '更多',
      loading: '加载中',
      farm: '猪场',
      confirmDeleteRecord: '确定要删除该条记录？',
      success: '操作成功',
      failed: '操作失败，请稍后再试',
      noData: '暂无数据',
      emptyRuleTitle: '暂无场景联动',
      emptySceneTitle: '暂无场景',
      emptyDescription: '创建规则，让设备自动响应条件变化'
    },
    login: {
      title: '物联网平台',
      emailPlaceholder: '请输入邮箱',
      passwordPlaceholder: '请输入密码',
      loginButton: '登录',
      registerButton: '免费注册',
      cancelLogin: '取消登录',
      loginSuccess: '登录成功',
      networkError: '网络错误，请稍后再试',
      loading: '加载中',
      authDescription: '登录',
      noAccount: '没有账号？',
      errors: {
        emailRequired: '请输入邮箱',
        invalidEmail: '请输入正确的邮箱格式',
        passwordRequired: '请输入密码',
        passwordTooShort: '密码至少需要6个字符',
        emailOrPasswordError: '邮箱或密码错误',
        userNotFound: '用户不存在',
        accountDisabled: '账号已被禁用',
        loginFailed: '登录失败，请稍后再试',
        unauthorized: '未授权，请重新登录'
      },
    },
    register: {
      title: '注册',
      subtitle: '创建您的 ThingsPanel 账号',
      hasAccount: '已经有账号？',
      emailPlaceholder: '请输入邮箱',
      codePlaceholder: '请输入验证码',
      getCode: '获取验证码',
      phonePlaceholder: '请输入手机号',
      passwordPlaceholder: '请输入密码',
      confirmPasswordPlaceholder: '请确认密码',
      registerButton: '确认注册',
      backToLogin: '返回登录',
      errors: {
        emailRequired: '请输入邮箱',
        invalidEmail: '请输入正确的邮箱格式',
        phoneRequired: '请输入手机号',
        invalidPhone: '请输入正确的手机号',
        passwordRequired: '请输入密码',
        passwordLength: '密码长度需在8-18位之间',
        passwordComplexity: '密码必须包含英文字母和数字',
        confirmPasswordRequired: '请确认密码',
        passwordMismatch: '两次输入的密码不一致',
        codeRequired: '请输入验证码'
      },
      messages: {
        codeSent: '验证码已发送',
        codeSending: '发送中...',
        codeFailed: '验证码发送失败',
        registerSuccess: '注册成功',
        registerFailed: '注册失败'
      }
    },
    alarms: {
      noAlerts: '暂无新的告警内容',
      alertLevel: '告警级别',
      alertName: '告警名称',
      alertContent: '告警原因',
      alertTime: '告警时间',
      alarmLevels: {
        L: '低',
        M: '中', 
        H: '高',
        '1': '高',
        '2': '中',
        '3': '低',
        default: '未知'
      },
      alarmStatus: '告警状态',
      alarmStatuses: {
        N: '正常',
        L: '低级报警',
        M: '中级报警',
        H: '高级报警',
        default: '未知状态',
      },
      sceneLinkageName: '关联场景联动名称',
      alarmDescription: '告警描述',
      alarmDevices: '告警设备列表',
      noDevices: '暂无关联设备',
      deviceID: '设备 ID',
      deviceName: '设备名称',
    },
    sceneAutomationEditor: {
      needAction: "请至少添加一个动作",
      finishAction: "请完成第 {index} 个动作配置",
      deviceActionsOnly: "场景管理仅支持操作设备动作",
      configureAction: "点击配置执行动作",
      actionOptions: "操作设备、激活场景或触发告警",
      on: "开启",
      off: "关闭",
      setValue: "设为 {value}",
      completeAction: "请完成动作配置",
      completeCurrentAction: "请先完成当前动作配置",
      configureCondition: "点击配置触发条件",
      conditionOptions: "设备、时间或状态变化",
      deviceEvent: "设备事件",
      completeCondition: "请完成条件配置",
      completeTime: "请完成时间配置",
      hourMinute: "第 {minute} 分钟",
      monthDay: "每月 {day} 日 {time}",
      selectWeek: "请选择星期",
      weekdays: "周一至周五",
      needCondition: "请至少添加一个条件",
      finishGroup: "请完成第 {group} 组条件",
      finishCondition: "请完成第 {group} 组第 {index} 个条件",
      rangeOnly: "同一条件组不能全部为时间范围",
      hourUnit: "时",
      minuteUnit: "分",
      minuteValue: "{minute} 分钟",
      ruleInfo: "规则信息",
      matchAll: "满足全部",
      timeAlarmConflict: "时间条件不能与触发告警同时使用",
      executionTime: "执行时间",
      addCondition: "添加条件",
      addAction: "添加动作",

      ruleName: '规则名称',
      alertLevel: '告警级别',
      notificationGroup: '通知组', 
      repeatCount: '重复次数',
      alertDescription: '告警描述',
      levels: {
        low: '低',
        medium: '中',
        high: '高'
      },
      ruleDescription: '规则说明',
      loading: '加载中',
      enterRuleName: '请输入场景联动名称',
      enterRuleConditions: '请输入场景联动条件',
      enterRuleActions: '请输入场景联动动作',
      ifText: '如果',
      thenText: '那么',
      save: '保存',
      saveConfirm: '确定要保存数据吗？',
      and: '且',
      or: '或',
      status: '状态',
      online: '在线', 
      offline: '离线',
      conditionType: '条件类型',
      all: '全部',
      actionPlaceholder: '执行动作',
      deviceCondition: '设备条件',
      timeCondition: '时间条件',
      jsonFormat: '请输入JSON格式',
      singleDevice: '单个设备',
      singleDeviceType: '单类设备',
      equal: '等于',
      notEqual: '不等于',
      greaterThan: '大于',
      lessThan: '小于',
      greaterThanOrEqual: '大于等于',
      lessThanOrEqual: '小于等于',
      between: '介于',
      in: '包含在',
      everyHour: '每小时',
      everyDay: '每天',
      everyWeek: '每周',
      everyMonth: '每月',
      monday: '星期一',
      tuesday: '星期二',
      wednesday: '星期三',
      thursday: '星期四',
      friday: '星期五',
      saturday: '星期六',
      sunday: '星期日',
      actionType1: '操作设备',
      actionType2: '触发告警',
      actionType3: '激活场景',
      selectActionType: '请选择需要执行的动作类型',
      completeDeviceInfo: '请将“操作设备”信息补充完整',
      completeWarningInfo: '请将“触发告警”信息补充完整（“告警级别”为必填）',
      completeNotificationInfo: '请将“触发告警”信息补充完整（“通知组”为必填）',
      completeSceneInfo: '请将“激活场景”信息补充完整（“场景”为必填）',
      unknownActionType: '未知的操作类型',
      and: '并且',
      selectConditionType: '请选择条件类型',
      selectDeviceConditionType: '请选择设备条件类型',
      selectDevice: '请选择设备',
      selectDeviceType: '请选择设备类型',
      selectParameter: '请选择参数',
      selectOperator: '请选择操作符',
      commaSeparatedValues: '用逗号分隔多个值',
      minValue: '最小值',
      maxValue: '最大值',
      value: '值',
      eventParamExample: "参数，如：{'{'}'param1':1{'}'}",
      selectTimeConditionType: '请选择时间条件类型',
      selectDateTime: '请选择日期和时间',
      selectExpirationTime: '请选择过期时间',
      selectCycle: '请选择周期',
      selectMinute: '请选择分钟',
      selectTime: '请选择时间',
      selectDate: '请选择日期',
      selectStartTime: '开始时间',
      selectEndTime: '结束时间',
      singleTime: '单次',
      repeat: '重复',
      timeRange: '时间范围',
      fiveMinutes: '5分钟',
      tenMinutes: '10分钟',
      thirtyMinutes: '30分钟',
      oneHour: '1小时',
      oneDay: '1天',
      everyHour: '每小时',
      everyDay: '每天',
      everyWeek: '每周',
      everyMonth: '每月',
      selectRelation: '请选择“且/或”',
      timeConditionValidation: '时间条件的“单次”与“单次”、“单次”与“重复”、“重复”与“单次”只能使用“或”连接，请检查您的条件',
      selectConditionType: '请选择条件类型',
      completeDeviceCondition: '请将“设备条件”信息补充完整（“设备条件”中所有字段均为必填）',
      completeTimeCondition: '请将“时间条件”信息补充完整（“时间条件”中所有字段均为必填）',
      unknownConditionType: '未知的条件类型',
      editSceneLinkage: '编辑场景联动',
      addSceneLinkage: '新增场景联动',
    },
    sceneEditor: {
      editScene: '编辑场景',
      newScene: '新增场景',
      sceneTitle: '场景标题',
      sceneDescription: '场景描述',
      actions: '动作',
      save: '保存',
      saveConfirm: '确定要保存数据吗？',
      loading: '加载中',
      enterSceneTitle: '请输入场景标题',
      actionsEdit: {
        selectActionType: '选择动作类型',
        selectDeviceType: '选择设备类型',
        selectDevice: '选择设备',
        selectDeviceCategory: '选择单类设备',
        selectMetricType: '选择指标类型',
        selectMetric: '选择指标',
        examplePrefix: '例如：',
        selectScene: '选择场景',
        selectAlarm: '选择告警'
      }
    },
    sceneSelect: {
      placeholder: '场景'
    },
  },
  components: {
    popup: {
      title: '提示',
      content: '确认此评论通过审核吗？',
      cancel: '取消',
      confirm: '确定'
    },
    authorize: {
      info: '请点击下方【获取授权】使用智慧猪场',
      cancelBtn: '取消授权',
      getAuthBtn: '获取授权'
    },
    login: {
      welcome: '登录',
      welcomeSubtitle: '欢迎登录',
      accountPlaceholder: '请输入账号!',
      passwordPlaceholder: '请输入密码!',
      rememberPassword: '记住密码',
      loginButton: '登 录',
      forgotPassword: '忘记密码',
      loginSuccess: '登录成功'
    },
    device: {
      placeholder: '设备',
      selectTitle: '选择设备',
      test: '测试'
    },
    selectDevice: {
      group: '分组',
      operator: '操作符',
      propertyValue: '属性值',
      statusProperty: '状态/属性',
      onlineStatus: '上下线',
      property: '属性',
      noData: '无数据！',
      online: '上线',
      offline: '下线',
      onAndOff: '上下线'
    },
    selectTime: {
      triggerType: '触发方式',
      single: '单次',
      repeat: '重复',
      range: '范围',
      triggerDateTime: '触发日期时间',
      singleTip: '只执行一次，执行结束后条件消失',
      repeatCycle: '重复周期',
      startTime: '起始时间',
      endTime: '结束时间',
      everyHour: '每小时',
      everyDay: '每天',
      everyWeek: '每周',
      everyMonth: '每月',
      customCron: '自定义cron',
      hourTip: '例: 33:00" 表示每小时的第33分钟触发',
      dayTip: '例: "18:33" 表示每天的18:33触发',
      weekTip: '例: "周一, 18:33" 表示每周一的18:33触发',
      monthTip: '例: "1日, 9:00" 表示每月1日的09:00触发',
      day: '日',
      executionRule: '执行日期时间规则'
    },
    notifyDialog: {
      confirmText: '点击确定{action}警告',
      process: '处理',
      ignore: '忽略',
      optional: '选填',
      operationSuccess: '操作成功'
    },
  }
}
